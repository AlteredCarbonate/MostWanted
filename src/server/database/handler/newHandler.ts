...
export class PlayerHandler {
    ...
    *
    */
   public requestAccount(player: alt.Player, where = player.name) {
      return playerModel.findOne({ userName: where });
   }
   /**
    * Creates User Account
    */
   public async createAccount(player: alt.Player) {
      let _playerAccount = await this.requestAccount(player);
